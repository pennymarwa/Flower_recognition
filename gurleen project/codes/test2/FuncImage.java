...
public class FuncImage {
	...
	private static FuncImageFinal finalOutput = new FuncImageFinal();
	public static void bufferedImageShow(final ArrayList<ArrayList<HashMap>> imageList,final HashMap imgNames,JFrame f) {
		final JFrame fBack = new JFrame("Output images");
		...
		fBack.setBounds(0,0,(220*multiplierFactor)+100, 840);
		ImagePanel imp=new ImagePanel("C:/Users/A/Downloads/Image Processing/backgroundimages/suns.jpg");
		...
		if(h.size()>1)
		{
			title3.setText("Current final outputs");
		}
		else
		{
				...
				lbl.addMouseListener(new MouseAdapter()
				{
					public void mouseClicked(MouseEvent evt)
					{
						...
						if (count == 2)
						{
							...
							returnIndex = Integer.parseInt(lbl.getToolTipText());
							finalOutput.bufferedImageShow(imageList, imgNames, returnIndex);
							fBack.dispose();
						}
					}
				});
		...
		if(h.size()>1)
		{
			JOptionPane.showMessageDialog(null, "Please select the final output image by clicking on it.");
		}
